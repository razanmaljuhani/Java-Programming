        ...
        while (true) {
            ...
            if (selection.equalsIgnoreCase(book) || selection.equalsIgnoreCase(add) || selection.equalsIgnoreCase(exit)) {
                ...
                if (selection.equalsIgnoreCase(book)) {
                    do {
                        printMsg("departmentsClinic");
                        clinicType = readAndVerify();
                    } while (clinicType == 'q');
...
//Method 2 char readAndVerify .
    public static char readAndVerify() {
        Scanner input = new Scanner(System.in);
        char clinicLetter = input.next().charAt(0);
        clinicLetter = Character.toUpperCase(clinicLetter);
        String allClinics = "SRID";
        if (allClinics.contains(clinicLetter + "")) {
            return clinicLetter;
        } else {
...
//Method 5  void  display info 1  . 
    public static void displayInfo(int id, String name, int time, String clinic, int fee) {
        ...
        System.out.printf("Clinic                  : %4s              Appointment Time           : %2s%n", clinic.toUpperCase(), displayTime(time));
...
//Method 6  void  display info 2  .    
    public static void displayInfo(int id, String name, int age, String phone, String clinic, double weight, double height) {
        ...
        System.out.printf("Clinic                    : %s %14s                   : %1.2f%2s%n", clinic.toUpperCase(), weightt, weight, unitkg);
        String bmi = bmi(weight, height);
        String unitCm = " cm";
        String heightt = "height";
        String bmii = "bmi";
        System.out.printf("%s                       : %s %15s                    : %1.2f%2s%n", bmii.toUpperCase(), bmi, heightt, height, unitCm);
